...
class PortfolioEnv(gym.Env):
    ...
    def __init__(self,
        ...
        pd.DataFrame(index=range(history.shape[1]))
    ...
    def plot(self):
        # show a plot of portfolio vs mean market performance
        df_info = pd.DataFrame(self.infos[1:])
        df_info['date'] = pd.to_datetime(df_info['date'], format='%Y-%m-%d')
        df_info.set_index('date', inplace=True)
        mdd = max_drawdown(df_info.rate_of_return + 1)
        sharpe_ratio = sharpe(df_info.rate_of_return)
        ...
        df_info[["mdd"]].plot(title='Max Dradowns', fig=plt.gcf(), rot=30)
        df_info[["sharpe_ratio"]].plot(title='Sharpe Ratio', fig=plt.gcf(), rot=30)
...
class MultiActionPortfolioEnv(PortfolioEnv):
    ...
    def _step(self, action):
        ...
        mean = df_info['return'].mean()
        std = df_info['return'].std()
    ...
    def plot(self):
        ...
        for model_name in self.model_names:
            plt.plot(df_info.index.values, df_info[model_name].values, label=model_name + ' Portfolio Value', color='red')
        plt.plot(df_info.index.values, df_info['market_value'].values, label='Market Value', color='blue')
        #plt.plot(df_info.index.values, df_info['sharpe_ratio'].values, label='sharpe_ratio', color='green')
        plt.xlabel('Date')
        plt.ylabel('Performance')
        plt.title('DDPG Portfolio Results')
        # plt.ylim(0.9, 2.5)
        plt.legend()
        plt.show()
